<?php
$fontfamilies = array(
  "texgyrepagella" => array(
    'R' => "TeXGyrePagella-Regular.ttf",
    'B' => "TeXGyrePagella-Bold.ttf",
    'I' => "TeXGyrePagella-Italic.ttf",
    'BI' => "TeXGyrePagella-BoldItalic.ttf",
  ),
  "droidserif" => array(
    'R' => "DroidSerif.ttf",
    'B' => "DroidSerif-Bold.ttf",
    'I' => "DroidSerif-Italic.ttf",
    'BI' => "DroidSerif-BoldItalic.ttf",
  ),
  "amiriserif" => array(
    'R' => "Amiri-Regular.ttf",
    'B' => "Amiri-Bold.ttf",
    'I' => "Amiri-Slanted.ttf",
    'BI' => "Amiri-BoldSlanted.ttf",
  ),
  "crimsontext" => array(
    'R' => "CrimsonTextregular.ttf",
    'B' => "CrimsonText700.ttf",
    'I' => "CrimsonTextitalic.ttf",
    'BI' => "CrimsonText700italic.ttf",
  ),
  "sourcesanspro" => array(
    'R' => "SourceSansPro-Regular.ttf",
    'B' => "SourceSansPro-Bold.ttf",
    'I' => "SourceSansPro-Italic.ttf",
    'BI' => "SourceSansPro-BoldItalic.ttf",
  ),
  "ubuntu" => array(
    'R' => "Ubuntu-R.ttf",
    'B' => "Ubuntu-B.ttf",
    'I' => "Ubuntu-RI.ttf",
    'BI' => "Ubuntu-BI.ttf",
  ),
  "ubuntu-light" => array(
    'R' => "Ubuntu-L.ttf",
    'B' => "Ubuntu-M.ttf",
    'I' => "Ubuntu-LI.ttf",
    'BI' => "Ubuntu-MI.ttf",
  ),
  "ubuntu-mono" => array(
    'R' => "UbuntuMono-R.ttf",
    'B' => "UbuntuMono-B.ttf",
    'I' => "UbuntuMono-RI.ttf",
    'BI' => "UbuntuMono-BI.ttf",
  ),
  "texgyrecursor" => array(
    'R' => "TeXGyreCursor-Regular.ttf",
    'B' => "TeXGyreCursor-Bold.ttf",
    'I' => "TeXGyreCursor-Italic.ttf",
    'BI' => "TeXGyreCursor-BoldItalic.ttf",
  ),
  "texgyreheros" => array(
    'R' => "TeXGyreHeros-Regular.ttf",
    'B' => "TeXGyreHeros-Bold.ttf",
    'I' => "TeXGyreHeros-Italic.ttf",
    'BI' => "TeXGyreHeros-BoldItalic.ttf",
  ),
  "texgyreschola" => array(
    'R' => "TeXGyreSchola-Regular.ttf",
    'B' => "TeXGyreSchola-Bold.ttf",
    'I' => "TeXGyreSchola-Italic.ttf",
    'BI' => "TeXGyreSchola-BoldItalic.ttf",
  ),
  "texgyrebonum" => array(
    'R' => "TeXGyreBonum-Regular.ttf",
    'B' => "TeXGyreBonum-Bold.ttf",
    'I' => "TeXGyreBonum-Italic.ttf",
    'BI' => "TeXGyreBonum-BoldItalic.ttf",
  ),
  "lato" => array(
    'R' => "Lato-Regular.ttf",
    'B' => "Lato-Bold.ttf",
    'I' => "Lato-Italic.ttf",
    'BI' => "Lato-BoldItalic.ttf",
  ),
  "lato-medium" => array(
    'R' => "Lato-Medium.ttf",
    'B' => "Lato-Semibold.ttf",
    'I' => "Lato-MediumItalic.ttf",
    'BI' => "Lato-SemiboldItalic.ttf",
  ),
  "lato-black" => array(
    'R' => "Lato-Semibold.ttf",
    'B' => "Lato-Black.ttf",
    'I' => "Lato-SemiboldItalic.ttf",
    'BI' => "Lato-BlackItalic.ttf",
  ),
  "lato-light" => array(
    'R' => "Lato-Light.ttf",
    'B' => "Lato-Regular.ttf",
    'I' => "Lato-LightItalic.ttf",
    'BI' => "Lato-Italic.ttf",
  ),
  "lato-thin" => array(
    'R' => "Lato-Thin.ttf",
    'B' => "Lato-Light.ttf",
    'I' => "Lato-ThinItalic.ttf",
    'BI' => "Lato-LightItalic.ttf",
  ),
  "roboto" => array(
    'R' => "Roboto-Regular.ttf",
    'B' => "Roboto-Bold.ttf",
    'I' => "Roboto-Italic.ttf",
    'BI' => "Roboto-BoldItalic.ttf",
  ),
  "roboto-black" => array(
    'R' => "Roboto-Bold.ttf",
    'B' => "Roboto-Black.ttf",
    'I' => "Roboto-BoldItalic.ttf",
    'BI' => "Roboto-BlackItalic.ttf",
  ),
  "roboto-light" => array(
    'R' => "Roboto-Light.ttf",
    'B' => "Roboto-Regular.ttf",
    'I' => "Roboto-LightItalic.ttf",
    'BI' => "Roboto-Italic.ttf",
  ),
  /* font crashes paperwhite:
  "roboto-thin" => array(
    'R' => "Roboto-Thin.ttf",
    'B' => "Roboto-Light.ttf",
    'I' => "Roboto-ThinItalic.ttf",
    'BI' => "Roboto-LightItalic.ttf",
  ),
  */
  "roboto-condensed" => array(
    'R' => "RobotoCondensed-Regular.ttf",
    'B' => "RobotoCondensed-Bold.ttf",
    'I' => "RobotoCondensed-Italic.ttf",
    'BI' => "RobotoCondensed-BoldItalic.ttf",
  ),
  "roboto-condensed-light" => array(
    'R' => "RobotoCondensed-Light.ttf",
    'B' => "RobotoCondensed-Regular.ttf",
    'I' => "RobotoCondensed-LightItalic.ttf",
    'BI' => "RobotoCondensed-Italic.ttf",
  ),
  "im_fell_dw_pica_pro" => array(
    'R' => "IM_FELL_DW_Pica_PRO_Roman.ttf",
    'I' => "IM_FELL_DW_Pica_PRO_Italic.ttf",
  ),
  "dejavusans" => array(
    'R' => "DejaVuSans.ttf",
    'B' => "DejaVuSans-Bold.ttf",
    'I' => "DejaVuSans-Oblique.ttf",
    'BI' => "DejaVuSans-BoldOblique.ttf",
  ),
  "dejavuserif" => array(
    'R' => "DejaVuSerif.ttf",
    'B' => "DejaVuSerif-Bold.ttf",
    'I' => "DejaVuSerif-Italic.ttf",
    'BI' => "DejaVuSerif-BoldItalic.ttf",
  ),
  "dejavusanscondensed" => array(
    'R' => "DejaVuSansCondensed.ttf",
    'B' => "DejaVuSansCondensed-Bold.ttf",
    'I' => "DejaVuSansCondensed-Oblique.ttf",
    'BI' => "DejaVuSansCondensed-BoldOblique.ttf",
  ),
  "dejavusansmono" => array(
    'R' => "DejaVuSansMono.ttf",
    'B' => "DejaVuSansMono-Bold.ttf",
    'I' => "DejaVuSansMono-Oblique.ttf",
    'BI' => "DejaVuSansMono-BoldOblique.ttf",
  ),
  "dejavuserifcondensed" => array(
    'R' => "DejaVuSerifCondensed.ttf",
    'B' => "DejaVuSerifCondensed-Bold.ttf",
    'I' => "DejaVuSerifCondensed-Italic.ttf",
    'BI' => "DejaVuSerifCondensed-BoldItalic.ttf",
  ),
  "texgyreadventor" => array(
    'R' => "TeXGyreAdventor-Regular.ttf",
    'B' => "TeXGyreAdventor-Bold.ttf",
    'I' => "TeXGyreAdventor-Italic.ttf",
    'BI' => "TeXGyreAdventor-BoldItalic.ttf",
  ),
  "texgyreheroscondensed" => array(
    'R' => "TeXGyreHerosCondensed-Regular.ttf",
    'B' => "TeXGyreHerosCondensed-Bold.ttf",
    'I' => "TeXGyreHerosCondensed-Italic.ttf",
    'BI' => "TeXGyreHerosCondensed-BoldItalic.ttf",
  ),
/* Amnesty Latin fonts */
  "amnestyhelveticaworld" => array(
    'R' => "AmnestyHelveticaWorld-Reg.ttf",
    'B' => "AmnestyHelveticaWorld-Bold.ttf",
    'I' => "AmnestyHelveticaWorld-Ita.ttf",
    'BI' => "AmnestyHelveticaWorld-BoldI.ttf",
    ),
  "amnestytradegothic" => array(
    'R' => "AmnestyTradeGothic.ttf",
    'B' => "AmnestyTradeGothic-Bd2.ttf",
    'I' => "AmnestyTradeGothic-Obl.ttf",
    'BI' => "AmnestyTradeGothic-Bd2Obl.ttf",
    ),
  "amnestytradegothicbdcn20" => array(
    'R' => "AmnestyTradeGothic.ttf",
    'B' => "AmnestyTradeGothic-BdCn20.ttf",
    'I' => "AmnestyTradeGothic-Obl.ttf",
    'BI' => "AmnestyTradeGothic-BdCn20Ob.ttf",
    ),
  "amnestytradegothiccn18" => array(
    'R' => "AmnestyTradeGothic-Cn18.ttf",
    'B' => "AmnestyTradeGothic-BdCn20.ttf",
    'I' => "AmnestyTradeGothic-Cn18Obl.ttf",
    'BI' => "AmnestyTradeGothic-BdCn20Ob.ttf",
    ),
  "amnestytradegothiclight" => array(
    'R' => "AmnestyTradeGothic-Light.ttf",
    'B' => "AmnestyTradeGothic-Bd2.ttf",
    'I' => "AmnestyTradeGothic-LightObl.ttf",
    'BI' => "AmnestyTradeGothic-Bd2Obl.ttf",
    ),
	"freesans" => array(
		'R' => "FreeSans.ttf",
		'B' => "FreeSansBold.ttf",
		'I' => "FreeSansOblique.ttf",
		'BI' => "FreeSansBoldOblique.ttf",
		'useOTL' => 0xFF,
		),
	"freeserif" => array(
		'R' => "FreeSerif.ttf",
		'B' => "FreeSerifBold.ttf",
		'I' => "FreeSerifItalic.ttf",
		'BI' => "FreeSerifBoldItalic.ttf",
		'useOTL' => 0xFF,
		'useKashida' => 75,
		),
	"freemono" => array(
		'R' => "FreeMono.ttf",
		'B' => "FreeMonoBold.ttf",
		'I' => "FreeMonoOblique.ttf",
		'BI' => "FreeMonoBoldOblique.ttf",
		),
  /*
  "" => array(
    'R' => "",
    'B' => "",
    'I' => "",
    'BI' => "",
  ),
  */
);
ksort($fontfamilies);
?>